...
export class ChartService {
  ...
  async createChart(): Promise<any> {


    const data = {
      labels: [
        "Transmission", "Stuffiness", "Discomfort", "Humidity", "Pollution", "Temperature", "CO2", "Density"
    ...
    for (let i = 0; i < data.datasets.length; i++) {
      ...
      const createdChart = new this.chartModel(data.datasets[i]);


      const Location = await createdChart.save();
    }

    // const createdChart = new this.chartModel(body);
    //
    //
    // const Location = await createdChart.save();
    return Chart;
